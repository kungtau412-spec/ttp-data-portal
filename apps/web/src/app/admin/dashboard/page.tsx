...
function AdminDashboardContent() {
  ...
  // Helpers to look up breakdown by type for a given region
  ...
  // Global supplier category data (filter to 4 required types)
    ...
    <div className="space-y-6">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div>
          <div className="flex items-center gap-2 mb-1">
            ...
            <span className="text-xs font-bold uppercase tracking-widest text-blue-500">Admin</span>
          </div>
          <h2 className="text-2xl font-bold text-[#344E41]">
            {greeting}, {firstName}
          </h2>
          <p className="text-slate-500 text-sm mt-1">TTP Data management &amp; mill operations</p>
        </div>
      ...
      {stats?.regionalStats && (
        <div className="space-y-4">
          ...
          {/* 6 summary cards (Issue 5): East Mills, West Mills, East Suppliers, West Suppliers, East Volume, West Volume */}
      ...
      <Card className="border border-slate-200 shadow-sm">
        <CardHeader className="pb-2">
          ...
          <p className="text-xs text-slate-400">Your management tools</p>
        </CardHeader>
        <CardContent>
          <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
            {quickActions.map((action) => (
              <Link
                key={action.label}
                href={action.href}
                className={`flex items-center gap-2.5 rounded-xl p-3 transition ${action.cls}`}
              >
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
              ...
              <div>
                <ResponsiveContainer width="100%" height={180}>
                  <BarChart
                    data={[
                      { name: 'RSPO', count: stats.certificationStats.rspo_count || 0 },
                      { name: 'MSPO', count: stats.certificationStats.mspo_count || 0 },
                      { name: 'ISCC', count: stats.certificationStats.iscc_count || 0 },
                      { name: 'None', count: stats.certificationStats.none_count || 0 },
                    ]}
                    margin={{ top: 4, right: 8, left: -20, bottom: 0 }}
                  >
                    <CartesianGrid strokeDasharray="3 3" stroke="#F1F5F9" />
                    ...
                    />
                    <Tooltip
                      formatter={(value: any) => [value, 'Suppliers']}
                      contentStyle={{
                        borderRadius: '8px',
                        border: '1px solid #E2E8F0',
                        fontSize: '12px',
                    ...
                    />
                    <Bar dataKey="count" fill="#588157" radius={[4, 4, 0, 0]} />
                  </BarChart>
                </ResponsiveContainer>
              </div>
            </div>
